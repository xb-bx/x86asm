...
cmp_reg64_reg64 :: proc(using assembler: ^Assembler, dest: Reg64, src: Reg64) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp %s, %s", dest, src)) }
...
cmp_reg8_memory :: proc(using assembler: ^Assembler, dest: Reg8, src: Memory) {
    ...
    rex: RexPrefix = {}
    if idest >= u8(Reg8.Spl) && idest <= u8(Reg8.Dil) { rex = {.Rex}}
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp %s, byte %v", dest, src)) }
    if idest > 15 { idest -= 12 }
...
cmp_reg32_memory :: proc(using assembler: ^Assembler, dest: Reg32, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp %s, dword %v", dest, src)) }
...
cmp_reg16_memory :: proc(using assembler: ^Assembler, dest: Reg16, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp %s, word %v", dest, src)) }
...
cmp_reg64_memory :: proc(using assembler: ^Assembler, dest: Reg64, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp %s, qword %v", dest, src)) }
...
cmp_memory_reg8 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg8) {
    ...
    if isrc >= u8(Reg8.Spl) && isrc <= u8(Reg8.Dil) { rex = {.Rex}}
    if isrc > 15 { isrc -= 12 }
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp byte %v, %s", dest, src)) }
...
cmp_memory_reg16 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg16) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp word %v, %s", dest, src)) }
...
cmp_memory_reg32 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg32) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp dword %v, %s", dest, src)) }
...
cmp_memory_reg64 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg64) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp qword %v, %s", dest, src)) }
...
cmp_memory_imm8 :: proc(using assembler: ^Assembler, dest: Memory, src: u8) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp byte %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp byte %v, %i", dest, src)) }
    }
...
cmp_memory_imm16 :: proc(using assembler: ^Assembler, dest: Memory, src: i16) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp word %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp word %v, %i", dest, src)) }
    }
...
cmp_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp dword %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp dword %v, %i", dest, src)) }
    }
...
cmpsx_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp qword %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("cmp qword %v, %i", dest, src)) }
    }
