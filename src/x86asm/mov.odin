...
mov_reg64_reg64 :: proc(using assembler: ^Assembler, dest: Reg64, src: Reg64) {
    if remember { append(&mnemonics, fmt.aprintf("mov %s, %s", dest, src)) }
...
mov_reg8_reg8 :: proc(using assembler: ^Assembler, dest: Reg8, src: Reg8) {
    ...
    if isrc > 15 { isrc -= 12 }
    generic_reg_or_imm_to_reg(assembler, rex, nil, mov_r8_rm8, REGISTER_DIRECT, int(idest), int(isrc), 0, 0, OperandEncoding.RM) 
}
mov_reg8_memory :: proc(using assembler: ^Assembler, dest: Reg8, src: Memory) {
    ...
    rex: RexPrefix = {}
    if idest >= u8(Reg8.Spl) && idest <= u8(Reg8.Dil) { rex = {.Rex}}
    if remember { append(&mnemonics, fmt.aprintf("mov %s, byte %v", dest, src)) }
    if idest > 15 { idest -= 12 }
...
mov_reg32_memory :: proc(using assembler: ^Assembler, dest: Reg32, src: Memory) {
    if remember { append(&mnemonics, fmt.aprintf("mov %s, dword %v", dest, src)) }
...
mov_reg16_memory :: proc(using assembler: ^Assembler, dest: Reg16, src: Memory) {
    if remember { append(&mnemonics, fmt.aprintf("mov %s, word %v", dest, src)) }
...
mov_reg64_memory :: proc(using assembler: ^Assembler, dest: Reg64, src: Memory) {
    if remember { append(&mnemonics, fmt.aprintf("mov %s, qword %v", dest, src)) }
...
mov_memory_reg8 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg8) {
    ...
    if isrc >= u8(Reg8.Spl) && isrc <= u8(Reg8.Dil) { rex = {.Rex}}
    if isrc > 15 { isrc -= 12 }
    if remember { append(&mnemonics, fmt.aprintf("mov byte %v, %s", dest, src)) }
...
mov_memory_reg16 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg16) {
    if remember { append(&mnemonics, fmt.aprintf("mov word %v, %s", dest, src)) }
...
mov_memory_reg32 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg32) {
    if remember { append(&mnemonics, fmt.aprintf("mov dword %v, %s", dest, src)) }
...
mov_memory_reg64 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg64) {
    if remember { append(&mnemonics, fmt.aprintf("mov qword %v, %s", dest, src)) }
...
mov_reg64_imm :: proc(using assembler: ^Assembler, dest: Reg64, src: int) {
    if src >= 10 {
        if remember { append(&mnemonics, fmt.aprintf("movabs %s, 0x%x", dest, src)) }
    }
    else {
        if remember { append(&mnemonics, fmt.aprintf("movabs %s, %i", dest, src)) }
    }
...
mov_memory_imm8 :: proc(using assembler: ^Assembler, dest: Memory, src: u8) {
    if src >= 10 {
        if remember { append(&mnemonics, fmt.aprintf("mov byte %v, 0x%x", dest, src)) }
    }
    else {
        if remember { append(&mnemonics, fmt.aprintf("mov byte %v, %i", dest, src)) }
    }
...
mov_memory_imm16 :: proc(using assembler: ^Assembler, dest: Memory, src: i16) {
    if src >= 10 {
        if remember { append(&mnemonics, fmt.aprintf("mov word %v, 0x%x", dest, src)) }
    }
    else {
        if remember { append(&mnemonics, fmt.aprintf("mov word %v, %i", dest, src)) }
    }
...
mov_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if remember { append(&mnemonics, fmt.aprintf("mov dword %v, 0x%x", dest, src)) }
    }
    else {
        if remember { append(&mnemonics, fmt.aprintf("mov dword %v, %i", dest, src)) }
    }
...
movsx_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if remember { append(&mnemonics, fmt.aprintf("mov qword %v, 0x%x", dest, src)) }
    }
    else {
        if remember { append(&mnemonics, fmt.aprintf("mov qword %v, %i", dest, src)) }
    }
...
mov_reg_imm8 :: proc(using assembler: ^Assembler, dest: Reg8, src: u8) {
    if src >= 10 {
        if remember { append(&mnemonics, fmt.aprintf("mov %v, 0x%x", dest, src)) }
    }
    else {
        if remember { append(&mnemonics, fmt.aprintf("mov %v, %i", dest, src)) }
    }
    idest := u8(dest)
    rex := RexPrefix {}
    if idest >= u8(Reg8.Spl) && idest <= u8(Reg8.Dil) { rex = {.Rex}}
...
lea :: proc(using assembler: ^Assembler, dest: Reg64, src: Memory) {
    if remember { append(&mnemonics, fmt.aprintf("lea %s, %v", dest, src)) }
...
movzx_reg16_reg8 :: proc(using assembler: ^Assembler, dest: Reg16, src: Reg8) {
    ...
    if isrc >= u8(Reg8.Spl) && isrc <= u8(Reg8.Dil) { rex = {.Rex}}
    if isrc > 15 { isrc -= 12 }
    if remember { append(&mnemonics, fmt.aprintf("movzx %s, %s", dest, src)) }
...
movsx_reg16_reg8 :: proc(using assembler: ^Assembler, dest: Reg16, src: Reg8) {
    ...
    if isrc >= u8(Reg8.Spl) && isrc <= u8(Reg8.Dil) { rex = {.Rex}}
    if isrc > 15 { isrc -= 12 }
    if remember { append(&mnemonics, fmt.aprintf("movsx %s, %s", dest, src)) }
...
movsx_reg64_reg32 :: proc(using assembler: ^Assembler, dest: Reg64, src: Reg32) {
    if remember { append(&mnemonics, fmt.aprintf("movsxd %s, %s", dest, src)) }
...
movsx_reg64_memory8 :: proc(using assembler: ^Assembler, dest: Reg64, src: Memory) {
    if remember { append(&mnemonics, fmt.aprintf("movsx %s, byte %v", dest, src)) }
...
movsx_reg64_memory16 :: proc(using assembler: ^Assembler, dest: Reg64, src: Memory) {
    if remember { append(&mnemonics, fmt.aprintf("movsx %s, word %v", dest, src)) }
