...
sub_reg64_reg64 :: proc(using assembler: ^Assembler, dest: Reg64, src: Reg64) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub %s, %s", dest, src)) }
...
sub_reg8_reg8 :: proc(using assembler: ^Assembler, dest: Reg8, src: Reg8) {
    ...
    generic_reg_or_imm_to_reg(assembler, rex, nil, sub_r8_rm8, REGISTER_DIRECT, int(idest), int(src), 0, 0, OperandEncoding.RM) 
}
sub_reg8_memory :: proc(using assembler: ^Assembler, dest: Reg8, src: Memory) {
    ...
    rex: RexPrefix = {}
    if idest >= u8(Reg8.Spl) && idest <= u8(Reg8.Dil) { rex = {.Rex}}
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub %s, byte %v", dest, src)) }
    if idest > 15 { idest -= 12 }
...
sub_reg32_memory :: proc(using assembler: ^Assembler, dest: Reg32, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub %s, dword %v", dest, src)) }
...
sub_reg16_memory :: proc(using assembler: ^Assembler, dest: Reg16, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub %s, word %v", dest, src)) }
...
sub_reg64_memory :: proc(using assembler: ^Assembler, dest: Reg64, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub %s, qword %v", dest, src)) }
...
sub_memory_reg8 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg8) {
    ...
    if isrc >= u8(Reg8.Spl) && isrc <= u8(Reg8.Dil) { rex = {.Rex}}
    if isrc > 15 { isrc -= 12 }
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub byte %v, %s", dest, src)) }
...
sub_memory_reg16 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg16) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub word %v, %s", dest, src)) }
...
sub_memory_reg32 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg32) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub dword %v, %s", dest, src)) }
...
sub_memory_reg64 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg64) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub qword %v, %s", dest, src)) }
...
sub_memory_imm8 :: proc(using assembler: ^Assembler, dest: Memory, src: u8) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub byte %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub byte %v, %i", dest, src)) }
    }
...
sub_memory_imm16 :: proc(using assembler: ^Assembler, dest: Memory, src: i16) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub word %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub word %v, %i", dest, src)) }
    }
...
sub_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub dword %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub dword %v, %i", dest, src)) }
    }
...
subsx_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub qword %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub qword %v, %i", dest, src)) }
    }
...
sub_reg_imm8 :: proc(using assembler: ^Assembler, dest: Reg8, src: u8) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("sub %v, %i", dest, src)) }
    }
    idest := u8(dest)
    rex := RexPrefix {}
    if idest >= u8(Reg8.Spl) && idest <= u8(Reg8.Dil) { rex = {.Rex}}
