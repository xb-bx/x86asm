...
comiss_xmm_xmm :: proc(using assembler: ^Assembler, dest: Xmm, src: Xmm) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("comiss %s, %s", dest, src)) }
...
comisd_xmm_xmm :: proc(using assembler: ^Assembler, dest: Xmm, src: Xmm) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("comisd %s, %s", dest, src)) }
...
comisd_xmm_mem64 :: proc(using assembler: ^Assembler, dest: Xmm, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("comisd %s, xmmword %s", dest, src)) }
