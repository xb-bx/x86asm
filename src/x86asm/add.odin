...
add_reg64_reg64 :: proc(using assembler: ^Assembler, dest: Reg64, src: Reg64) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add %s, %s", dest, src)) }
...
add_reg8_reg8 :: proc(using assembler: ^Assembler, dest: Reg8, src: Reg8) {
    ...
    generic_reg_or_imm_to_reg(assembler, rex, nil, add_r8_rm8, REGISTER_DIRECT, int(idest), int(src), 0, 0, OperandEncoding.RM) 
}
add_reg8_memory :: proc(using assembler: ^Assembler, dest: Reg8, src: Memory) {
    ...
    rex: RexPrefix = {}
    if idest >= u8(Reg8.Spl) && idest <= u8(Reg8.Dil) { rex = {.Rex}}
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add %s, byte %v", dest, src)) }
    if idest > 15 { idest -= 12 }
...
add_reg32_memory :: proc(using assembler: ^Assembler, dest: Reg32, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add %s, dword %v", dest, src)) }
...
add_reg16_memory :: proc(using assembler: ^Assembler, dest: Reg16, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add %s, word %v", dest, src)) }
...
add_reg64_memory :: proc(using assembler: ^Assembler, dest: Reg64, src: Memory) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add %s, qword %v", dest, src)) }
...
add_memory_reg8 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg8) {
    ...
    if isrc >= u8(Reg8.Spl) && isrc <= u8(Reg8.Dil) { rex = {.Rex}}
    if isrc > 15 { isrc -= 12 }
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add byte %v, %s", dest, src)) }
...
add_memory_reg16 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg16) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add word %v, %s", dest, src)) }
...
add_memory_reg32 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg32) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add dword %v, %s", dest, src)) }
...
add_memory_reg64 :: proc(using assembler: ^Assembler, dest: Memory, src: Reg64) {
    if mnemonics != nil { append(&mnemonics, fmt.aprintf("add qword %v, %s", dest, src)) }
...
add_reg_imm8 :: proc(using assembler: ^Assembler, dest: Reg8, src: u8) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add %v, %i", dest, src)) }
    }
    idest := u8(dest)
    rex := RexPrefix {}
    if idest >= u8(Reg8.Spl) && idest <= u8(Reg8.Dil) { rex = {.Rex}}
...
add_memory_imm8 :: proc(using assembler: ^Assembler, dest: Memory, src: u8) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add byte %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add byte %v, %i", dest, src)) }
    }
...
add_memory_imm16 :: proc(using assembler: ^Assembler, dest: Memory, src: i16) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add word %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add word %v, %i", dest, src)) }
    }
...
add_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add dword %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add dword %v, %i", dest, src)) }
    }
...
addsx_memory_imm :: proc(using assembler: ^Assembler, dest: Memory, src: i32) {
    if src >= 10 {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add qword %v, 0x%x", dest, src)) }
    }
    else {
        if mnemonics != nil { append(&mnemonics, fmt.aprintf("add qword %v, %i", dest, src)) }
    }
